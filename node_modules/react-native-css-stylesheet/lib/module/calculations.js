...
export const sizeRegex = /^(?:(?:(\d+(?:\.\d+)?){1}((?:(mv?s)+(0*(?:\.\d+)|1(\.0+)){1}){1}|s|vs|px)?)|(?:(100|\d\d?(?:\.\d+)?){1}(rem|vh|vw|vmin|vmax)))$/;
...
export const scale = size => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
...
export const moderateScale = (size, factor = 0.5) => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
  const formattedFactor = isValidPositiveNumber(factor, "Factor should be a number.", "Factor should be greater than 0.");
...
export const viewportMin = size => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
  return formattedSize / 100 * getShortDimension();
};
export const viewportMax = size => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
  return formattedSize / 100 * getLongDimension();
};
export const viewportHeight = size => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
  return formattedSize / 100 * getHeight();
};
export const viewportWidth = size => {
  const formattedSize = isValidPositiveNumber(size, "Size should be a number.", "Size should be greater than 0.");
  return formattedSize / 100 * getWidth();
};
export const isValidSize = size => regexTest(sizeRegex, size).length > 0;
...
export const setGuidelineBaseDimensions = (guidelineBaseWidth = 0, guidelineBaseHeight = 0) => {
  const baseWidth = isValidPositiveNumber(guidelineBaseWidth, "Guideline Base Width should be a number.", "Guideline Base Width should be greater than 0.");
  const baseHeight = isValidPositiveNumber(guidelineBaseHeight, "Guideline Base Height should be a number.", "Guideline Base Height should be greater than 0.");
  defaultSize.guidelineBaseHeight = baseHeight;
  defaultSize.guidelineBaseWidth = baseWidth;
};
export const setDefaultFontSize = (defaultFontSize = 0, responsive) => {
  const fontSize = isValidPositiveNumber(defaultFontSize, "Default Font Size should be a number.", "Default Font Size should be greater than 0.");
