...
import { calculateSize, getCurrentOrientation, isValidSize } from "./calculations";
  ...
  if (error && Object.keys(error).length === 0 && results && results.length > 0) {
    for (let i = 0; i < results.length; i += 1) {
      ...
      for (let j = 0; j < group.length; j += 1) {
        ...
        if (condition === "orientation" && value === getCurrentOrientation()) {
          isGroupActive = isGroupActive && true;
        } else if ((condition === "min" || condition === "max") && (dimension === "height" || dimension === "width")) {
          const currentDimension = Dimensions.get("window")[dimension];
...
function calculateStyles(styles) {
  ...
  for (let i = 0; i < stylesKey.length; i += 1) {
    ...
    for (let j = 0; j < styleKey.length; j += 1) {
      ...
      const stylePropertyResult = regexTest(isResponsiveKey, styleProperty);
      let styleValue;
...
export { default as withCssStyleSheet, WithCssStyleSheetProps } from "./utils/withCssStyleSheet";
